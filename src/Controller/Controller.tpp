...
namespace Ramulator
{
  ...
  void Controller<T>::finish(long read_req, long dram_cycles)
  {
    ...
    if (rh_defense != nullptr) 
    { 
      additional_rank_refresh_sum = rh_defense->additional_rank_refreshes;
      rh_defense->finish();
    }
  ...
  void Controller<T>::reg_stats(uint num_cores)
  {
    ...
    additional_rank_refresh_sum
        .name("additional_rank_refresh_sum_"+to_string(channel->id) + "_core")
        .desc("Number of additional rank refreshes per channel per core")
        .precision(0)
        ;
