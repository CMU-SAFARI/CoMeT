...
namespace Ramulator
{
  ...
  template <class T>
  Paragraph<T>::Paragraph(const YAML::Node &config, Controller<T>* ctrl) : RefreshBasedDefense<T>(config, ctrl)
  {
  ...
  template <class T>
  void Paragraph<T>::update(typename T::Command cmd, const std::vector<int> &addr_vec, uint64_t open_for_nclocks, int core_id)
  {
  ...
  template <class T>
  void Paragraph<T>::completely_refresh_dram_rank(int no_ranks)
  {
  ...
  template <class T>
  void Paragraph<T>::refresh_only_accessed_rows(int no_ranks)
  {
