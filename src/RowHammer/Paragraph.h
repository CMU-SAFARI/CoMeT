...
{
  template <class T>
  class Paragraph : public RefreshBasedDefense<T>
  {
  public:
    Paragraph(const YAML::Node &config, Controller<T>* ctrl);
    ~Paragraph() = default;
    void tick();
  ...
  private:
    ...
    // probability threshold for Paragraph
