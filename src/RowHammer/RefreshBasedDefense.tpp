...
namespace Ramulator{  
  ...
  RefreshBasedDefense<T>* make_refresh_based_defense(YAML::Node config, Controller<T>* ctrl)
  {
    ...
    else if (rbd_type == "Paragraph")
      rbd = new Paragraph<T>(rbd_config, ctrl);
    else if (rbd_type == "CoMeT")
      rbd = new CoMeT<T>(rbd_config, ctrl);
    else if (rbd_type == "OracleCounterBased")
      rbd = new OracleCounterBased<T>(rbd_config, ctrl);
    else if (rbd_type == "ActCounter")
      rbd = new ActCounter<T>(rbd_config, ctrl);
